...
class RouteViewController: UIViewController, CLLocationManagerDelegate, GMSMapViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.delegate = containerVC.routeViewController.delegate
    ...
    override func observeValueForKeyPath(keyPath: String?, ofObject object: AnyObject?, change: [String : AnyObject]?, context: UnsafeMutablePointer<Void>) {
        if !didFindMyLocation {
            let myLocation: CLLocation = (change! as [NSObject : AnyObject])[NSKeyValueChangeNewKey] as! CLLocation
...
extension RouteViewController: SidePanelViewControllerDelegate {
    
    func presentViewControl(indexPassed: NSIndexPath)
    {
        var destViewController : UIViewController!
        switch (indexPassed.row) {
        case 0: destViewController  = self.storyboard?.instantiateViewControllerWithIdentifier("ViewController") as! ViewController
            break
        case 1:
            var centerNavigationController: UINavigationController!
            destViewController  = self.storyboard?.instantiateViewControllerWithIdentifier("RouteViewController") as! RouteViewController
            centerNavigationController = UINavigationController(rootViewController: destViewController)
            var check = false
            check = (self.navigationController?.topViewController?.isKindOfClass(RouteViewController))!
            if !check
            {
                view.addSubview(centerNavigationController.view)
                addChildViewController(centerNavigationController)
                print(self.navigationController?.topViewController?.isKindOfClass(RouteViewController))
        ...
        default: print("Under Processing")
        delegate?.collapseSidePanels()
